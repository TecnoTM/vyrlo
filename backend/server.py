...
async def exchange_session(request: Request, response: Response):
    """Exchange Supabase access token for session"""
    ...
    name = data.get("user_metadata", {}).get("full_name") or email
    ...
    await db.user_sessions.replace_one({"user_id": user_id}, session_doc, upsert=True)
    ...
    try:
        ...
        if transaction["payment_status"] != new_status:
            await db.payment_transactions.update_one(
                ...
                {"$set": {"payment_status": new_status, "stripe_payment_intent_id": checkout_session.payment_intent, "updated_at": datetime.now(timezone.utc)}}
            )
            await db.bookings.update_one(
                {"booking_id": transaction["booking_id"]},
                {"$set": {"status": booking_status, "payment_status": new_status}}
            )
        booking = await db.bookings.find_one({"booking_id": transaction["booking_id"]}, {"_id": 0})
        item = await db.items.find_one({"item_id": transaction["item_id"]}, {"_id": 0})
        return {
...
async def send_message(message_data: MessageCreate, request: Request):
    ...
    if not existing_conv:
        ...
        await db.conversations.update_one(
            {"conversation_id": conversation_id},
            {"$set": {"last_message": message_data.content[:100], "last_message_at": datetime.now(timezone.utc)}}
        )
    message = Message(
        conversation_id=conversation_id,
    ...
    return MessageResponse(**message.model_dump(), sender_name=user.name, sender_picture=user.picture)

@api_router.get("/conversations", response_model=List[ConversationResponse])
async def get_conversations(request: Request):
    user = await require_auth(request)
    conversations = await db.conversations.find({"participants": user.user_id}, {"_id": 0}).sort("last_message_at", -1).to_list(100)
    result = []
    for conv in conversations:
        ...
        unread_count = await db.messages.count_documents({"conversation_id": conv["conversation_id"], "receiver_id": user.user_id, "read": False})
        item_title = None
        item_image = None
        if conv.get("item_id"):
...
async def get_conversation_messages(conversation_id: str, request: Request):
    ...
    for msg in messages:
        ...
        result.append(MessageResponse(**msg, sender_name=sender.get("name") if sender else None, sender_picture=sender.get("picture") if sender else None))
    return result
...
async def start_conversation(request: Request):
    ...
    if not existing:
        conv = Conversation(conversation_id=conversation_id, participants=[user.user_id, receiver_id], item_id=item_id)
        await db.conversations.insert_one(conv.model_dump())
    if message:
        msg = Message(conversation_id=conversation_id, sender_id=user.user_id, receiver_id=receiver_id, content=message)
        await db.messages.insert_one(msg.model_dump())
        await db.conversations.update_one({"conversation_id": conversation_id}, {"$set": {"last_message": message[:100], "last_message_at": datetime.now(timezone.utc)}})
    ...
    test_items = [
        {
            ...
            "price": 45.0,
            "description": "Drone professionale sotto i 250g, riprese 4K HDR. Batteria extra inclusa.",
        ...
        },
        {
            ...
            "price": 5.0,
            "description": "7a edizione dell'Atlante Netter. Libro fondamentale per studenti di medicina.",
    ...
    demo_users = [
        {
            ...
            "description": "Appassionato di fotografia e tecnologia.",
            "location": "Milano, IT",
            "created_at": datetime.now(timezone.utc)
        },
        {
